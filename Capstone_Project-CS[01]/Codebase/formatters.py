...
class ResumePDFFormatter:
    ...
    def __init__(self):
        ...
        self.pdf = None
        self.font_family = self._get_os_font()
    
    def _get_os_font(self):
        """Get appropriate font based on operating system"""
        system = platform.system()
        
        if system == 'Windows':
            # Windows: Arial is standard
            return 'Arial'
        elif system == 'Darwin':
            # macOS: Arial is available
            return 'Arial'
        elif system == 'Linux':
            # Linux: Use core PDF fonts (Helvetica) for compatibility
            # Arial may not be available without additional font packages
            return 'Helvetica'
        else:
            # Fallback to core PDF font
            return 'Helvetica'
